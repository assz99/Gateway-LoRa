...
void OnMessage(sio::event& ev)
{
  ...
  debug("\tChegou uma mensagem do Servidor\n");
  SendMessageLoRa(socketmsg);
}
...
int main(int argc, char *argv[])
{
  ...
  cout << "\033[1;33mUltima atualizacao: " << LAST_UPDATE << "\033[0m" << endl
       << endl
       << endl;
  ...
  h.connect("http://192.168.0.3:8001");

  h.socket()->on("LoRamessage", &OnMessage);

  /*h.socket()->on("LoRamessage", [&](sio::event& ev)
{
  std::string socketmsg = ev.get_message()->get_string();
  debug("\tChegou uma mensagem do Servidor\n");
  debug(ui.verde + socketmsg + ui.remove_cor);
  //SendMessageLoRa();
});*/
  ...
  while (!force_exit)
  {
    ...
    radioNumber = radioNumber > number_of_radios_to_initialize - 1 ? 0 : radioNumber + 1;
